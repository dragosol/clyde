...
**3. Scaffolding crowds out the task.** A 24 KB system prompt expanded to 8–15K tokens per turn once tool schemas, memory sections, the graph section and skill hints were templated in. Worse, small models never used the graph, the layered memory, or skill switching — so most of that budget was spent on features they ignored. Ship a compact prompt mode for small backends.
...
Clyde is free. If the harness or the write-up saved you time:
